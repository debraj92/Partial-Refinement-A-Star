...
void AbstractGraph_6::createAbstractGraphNodes() {
    auto &abGraph4Map = abGraph5.accessAbstractGraph();
    ulonglong color = 0;
    for(ulonglong abGColor = 1; abGColor <= abGraph4Map.size(); ++abGColor) {
        ...
        for(auto connected1_itr = abNode.reachableNodes.begin(); connected1_itr != abNode.reachableNodes.end(); ++connected1_itr) {
            auto &abNode1 = abGraph5.unrank(*connected1_itr);
            if (abNode.totalNodesInRepresentation + abNode1.totalNodesInRepresentation > MAX_NODES) continue;

            double edge1 = abGraph5.findShortestDistanceBetweenNodes(abNode1, abNode);
            ...
            */
            ...
            for(auto connected2_itr = abNode.reachableNodes.begin(); connected2_itr != abNode.reachableNodes.end(); ++connected2_itr) {
                ...
                if (skip) continue;
                auto &abNode2 = abGraph5.unrank(*connected2_itr);
                ...
                */
                for(auto connected3_itr = abNode2.reachableNodes.begin(); connected3_itr != abNode2.reachableNodes.end(); ++connected3_itr) {
                    ...
                    auto &abNode3 = abGraph5.unrank(*connected3_itr);
        ...
        if (!found4Clique) {
            ...
            if (found3Clique) {
                auto &node1 = abGraph5.unrank(three_clique[0]);
                auto &node2 = abGraph5.unrank(three_clique[1]);
                auto &node3 = abGraph5.unrank(three_clique[2]);
                ...
                auto &node1 = abGraph5.unrank(two_clique[0]);
                auto &node2 = abGraph5.unrank(two_clique[1]);
...
void AbstractGraph_6::dfsToConnectAbstractNodes(const AbstractNode &abNode, ulonglong abG6Color, unordered_set<ulonglong> &visited) {
    if (!colorAbstractNodeMap.contains(abG6Color)) {
        return;
    }
    ...
    for(const auto& connectedChildNodeColor : abNode.reachableNodes) {
        auto &connectedChildNode = abGraph5.unrank(connectedChildNodeColor);
        assert(connectedChildNode.abstractionColor > 0);
        if (connectedChildNode.abstractionColor == abG6Color) {
            dfsToConnectAbstractNodes(connectedChildNode, abG6Color, visited);
        } else {
            createUndirectedEdge(abG6Color, connectedChildNode.abstractionColor);
        }
    }
}

void AbstractGraph_6::createUndirectedEdges() {
    ulonglong abG6Color = 1;
    unordered_set<ulonglong> visited;
    while (colorAbstractNodeMap.contains(abG6Color)) {
        const auto &centroid = colorAbstractNodeMap[abG6Color].centroidRealNode;
        ...
        if (!visited.contains(abG5Color)) {
            const auto &abG5Node = abGraph5.unrank(abG5Color);
            dfsToConnectAbstractNodes(abG5Node, abG6Color, visited);
        }
        //printNode(abG6Color);
        ++abG6Color;
    }
...
void AbstractGraph_6::createUndirectedEdge(ulonglong color1, ulonglong color2) {
...
void AbstractGraph_6::setGoalColor(ulonglong color) {
    goalColor = color;
}

Node AbstractGraph_6::createNode(ulonglong color) {
    return {(ulonglong) color};
}

bool AbstractGraph_6::isGoalReached(ulonglong color) {
    return goalColor == color;
}

AbstractNode &AbstractGraph_6::unrank(ulonglong rank) {
    assert(colorAbstractNodeMap.find(rank) != colorAbstractNodeMap.end());
    return colorAbstractNodeMap.find(rank)->second;
}

ulonglong AbstractGraph_6::getGoalColor() {
    return goalColor;
}

vector<AbstractNode> AbstractGraph_6::getAllAbstractNodes() {
...
void AbstractGraph_6::createAbstractGraph() {
    createAbstractGraphNodes();
    createUndirectedEdges();
}

void AbstractGraph_6::printNode(ulonglong color) {
...
ulonglong AbstractGraph_6::getStartColor() {
    int startX, startY;
    ...
    return abGraph5.unrank(startColor_abstraction5).abstractionColor;
}

unordered_map<ulonglong, AbstractNode> &AbstractGraph_6::accessAbstractGraph() {
    return colorAbstractNodeMap;
}

void AbstractGraph_6::printConnectedColors() {
